...
func (postRepo) FindAll() ([]model.Post, error) {
	...
	// 	Find(&posts)
	result := db.Joins("User").Preload("Playlist.WorkoutProfile.WorkoutMusictype.MusicType").Order("p_datetime DESC").Find(&posts)
	if result.Error != nil {
...
func (postRepo) FindByID(id int) ([]model.Post, error) {
	posts := []model.Post{}
	result := db.Preload("Playlist.WorkoutProfile.WorkoutMusictype.MusicType").Where("uid = ?", id).Order("p_datetime DESC").Find(&posts)
	if result.Error != nil {
		return nil, result.Error
	}
	return posts, nil
